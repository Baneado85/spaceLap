...
import { QrCode, CheckCircle2, Clock, XCircle, AlertCircle, Trash2, X } from 'lucide-react';
...
export const RequestsHistory: React.FC<RequestsHistoryProps> = ({ requests, onCancelRequest }) => {
  ...
  const filteredRequests = requests.filter((req) => {
    if (filter === 'active') return req.status === 'approved' || req.status === 'pending';
    ...
    <div className="flex-1 p-5 space-y-4 bg-[#F3F4F6] overflow-y-auto">
      ...
      {filteredRequests.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredRequests.map((req) => (
            <div
              key={req.id}
              className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 space-y-3 transition-all hover:shadow-md"
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-xs font-bold text-[#002B66]">{req.labName}</h3>
                  <p className="text-[11px] text-slate-500 font-medium">{req.building}</p>
                </div>
              ...
              <div className="text-xs text-slate-600 space-y-1 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                <p>
                  <span className="font-semibold text-slate-700">Fecha:</span> {req.date}
                </p>
                <p>
                  <span className="font-semibold text-slate-700">Horario:</span> {req.startTime} - {req.endTime}
                </p>
                <p className="text-[11px] italic text-slate-500">Motivo: {req.purpose}</p>
              </div>
      ...
      {selectedQRReq && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-xs shadow-2xl flex flex-col items-center text-center space-y-4 animate-in zoom-in-95 duration-150 relative">
            <button
            ...
            <h3 className="text-sm font-bold text-slate-900 font-heading pt-2">
              Pase Digital de Ingreso PUCP
            </h3>
            ...
            <div className="text-xs text-slate-600 space-y-1">
              <p className="font-bold text-[#002B66]">{selectedQRReq.labName}</p>
              <p className="text-[11px]">{selectedQRReq.date} ({selectedQRReq.startTime} - {selectedQRReq.endTime})</p>
